#define render_dialog_INTERNAL public
...
///
/// Invariants:
/// - If a job was canceled or ran into an error, then if finished == true, then
...
void RenderDialog::update_status() {
    ...
    // Don't query _backend since you won't get a consistent view of rendering state.
    // Only use variables calculated from job_progress.
    if (all_finished) {
        _done = true;
        _status_timer.stop();
